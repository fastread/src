...
class MAR(object):
    ...
    def get_numbers(self):
        ...
        print ("body len", len(self.body["code"]))
    ...
    def preprocess(self):
        ...
        print("Preprocess...")
        content = [self.body["Document Title"][index] + " " + self.body["Abstract"][index] for index in                     xrange(len(self.body["Document Title"]))]
        #######################################################
        ...
        for item in range(1,5):
            for i in len(self.csr_mat[item]):
                print (self.csr_mat[item,i])
        ########################################################
        return
    ...
    def train(self,pne=True,weighting=True):
        ...
        poses = np.where(np.array(self.body['code']) == "yes")[0]
        negs = np.where(np.array(self.body['code']) == "no")[0]
        print("poses ", len(poses), " negs ", len(negs))
        left = poses
